...
class SimplePath:
    def __init__(self):
        ...
        """
    def calc_linear_path(self, start_point, end_point, num_points):
        """
        ...
        for i in range(num_points):
            path[i, 0] = start_point[0] * (1-t[i]) + t[i] * end_point[0]
            path[i, 1] = start_point[1] * (1-t[i]) + t[i] * end_point[1]
            path[i, 2] = start_point[2] * (1-t[i]) + t[i] * end_point[2]
    ...
    def calc_circle_path(self, start_point, mid_point, end_point, num_points, rotation_angle=None, epsilon=1e-6):
        ...
        start_to_mid = mid_pose - start_point
        mid_to_end = end_point - mid_pose
        ...
        a = start_point-mid_pose #P0-P1
        b = mid_pose-end_point   #P1-P2
        c = end_point-start_point #P2-P0
        ...
        center = c0*start_point + c1*mid_pose + c2*end_point
...
if __name__ == "__main__":
    ...
    end_point = np.array([4, 0, 2])

    simple_path = SimplePath()

    linear_path = simple_path.calc_linear_path(start_point, end_point, 100)
    circular_path = simple_path.calc_circle_path(start_point, mid_pose, end_point, 100, rotation_angle=2*np.pi)
    ...
    ax.plot([start_point[0]], [start_point[1]], [start_point[2]], 'go', markersize=8, label='Inicio')
    ax.plot([end_point[0]], [end_point[1]], [end_point[2]], 'mo', markersize=8, label='Fin')
